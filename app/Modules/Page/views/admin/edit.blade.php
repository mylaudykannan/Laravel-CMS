...
@push('stylesheets')
    ...
    @section('content')
        <div class="container-fluid" id="editdiv">
            ...
            <div class="row">
                <div class="col-12">
                    <form action="{{ URL('admin/page/update?lang=' . app()->getLocale()) }}" method="post"
                        id="pageform">
                        @csrf
                        <div class="card customCard mt-4 mb-5" id="titlecard">
                            ...
                            <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                                <div
                                    ...
                                    <h5 class="text-white text-capitalize pl-3 mb-0">Edit Page</h5>
                                    <a href="{{ URL('admin/page/add') }}"
                                        class="btn btn-default mb-0 mr-3 btn-sm text-white">ADD NEW PAGE</a>
                                </div>
                            </div>
                            <div class="card-body pt-2">
                                <div class="settingsdiv">
                                    <span
                                        class="badge badge-secondary settingbtn @if ($locale == 'en') active @endif"
                                        onClick="window.location.href='{{ url()->current() }}'">
                                        English
                                    </span>
                                    <span
                                        class="badge badge-secondary settingbtn @if ($locale == 'ar') active @endif"
                                        onClick="window.location.href='{{ url()->current() }}?lang=ar'">
                                        Arabic
                                    </span>
                                    <span
                                        class="badge badge-secondary settingbtn @if ($locale == 'ru') active @endif"
                                        onClick="window.location.href='{{ url()->current() }}?lang=ru'">
                                        Russian
                                    </span>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control required" id="title" placeholder="Enter Title"
                                        name="title" value="{{ $page['title'] ?? '' }}" data-empty="Enter Title"
                                        data-max="250" data-lengtherr="Maximum 250 characters allowed" />
                                </div>
                                ...
                                placeholder="Enter Content" data-empty="Enter Content" data-allowed="page">{{$page['content'] ?? ''}}</textarea> --}}
                                    <textarea class="form-control" id="content" name="content" placeholder="Enter Content" data-empty="Enter Content">{{ $page['content'] ?? '' }}</textarea>
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control required" data-empty="Select Status" name="status"
                                        id="status">
                                        <option value="">Select Status</option>
                                        <option value="1" @if (isset($page['status']) && $page['status'] == '1') selected @endif>Enable</option>
                                        <option value="0" @if (isset($page['status']) && $page['status'] == '0') selected @endif>Disable
                                        </option>
                                    </select>
                                </div>
                                <div class="form-group">
                                    ...
                                    <input type="text" readonly class="form-control flatpicker required"
                                        value="{{ $puhdate }}" id="publish_on" placeholder="Enter Publish Date"
                                        name="publish_on" data-empty="Enter Publish Date" />
                                </div>
                                ...
                                <div class="card customCard transparent">
                                    ...
                                    <div class="card-body pt-2">
                                        ...
                                        <div class="">
                                            <div class="row clearfix">
                                                <div class="col-md-12 table-responsive">
                                                    <table id="tab_logic{{ $k }}"
                                                        ...
                                                        <tbody>
                                                            @if (
                                                                (config('page.section.' . $slug . '.' . $k . '.static') && empty($pagesections[$k])) ||
                                                                ...
                                                                <tr id="{{ $k }}addr0" data-id="0"
                                                                    ...
                                                                    <td data-name="stitle">
                                                                        ...
                                                                        @else
                                                                            @include(
                                                                                'Page::admin.section.fields',
                                                                                ['k' => $k, 'firstrow' => true]
                                                                            )
                                                                        @endif
                                                                    </td>
                                                                    @if (!config('page.section.' . $slug . '.' . $k . '.static'))
                                                                        <td data-name="del"><button
                                                                                name="{{ $k }}del0"
                                            ...
                                            @if (!config('page.section.' . $slug . '.' . $k . '.static'))
                                                <a id="add_row{{ $k }}"
                                                    class="btn btn-primary pull-left text-white add_row"
                                                    data-section="{{ $k }}">Add Row</a><input type="hidden"
                                                    name="loopcount{{ $k }}"
                                                    value="{{ count($pagesections[$k] ?? []) }}"
                                                    id="loopcount{{ $k }}">
                                            @endif
                        ...
                        <div class="d-flex submitbtndiv">
                            @include('admin.inc.updatenotes', ['form' => '#pageform'])
                            <a href="{{ URL('admin/page') }}" class="btn btn-success ml-3 mb-0">Back</a>
    ...
    @push('scripts')
        ...
        <script>
            ...
            $('body').on('click', '#draftsubmitbtn', function(e) {
                ...
                var result = $.ajax({
                    ...
                    url: root + '/admin/draftpage/update?frompage&lang=' + locale,
                    type: "POST",
                    datatype: "json",
        ...
        @if (!empty($page) && config('page.view.' . $slug))
            @include('Page::admin.view.' . $slug)
        @endif
